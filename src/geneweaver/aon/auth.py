...
AUTHORIZATIONS = {"Access": {"type": "apiKey", "in": "header", "name": "Authorization"}}
...
def get_decode_key_from_auth0(header):
    ...
    if rsa_key is None:
        ...
        LOGGER.warning(
            f"This key was NOT issued by {current_app.config['AUTH_DOMAIN']}!"
        )
    ...
    def wrapper(f):
        ...
        def check_scopes(*args, **kwargs):
            ...
            raise AuthError(
                {
                    "code": "missing_scopes",
                    "description": "Token does not contain the required scopes to access this resource",
                },
                401,
            )
