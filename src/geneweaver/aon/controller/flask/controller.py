...
algorithm_model = NS.model(
    "algorithms", {"alg_id": fields.Integer(), "alg_name": fields.String()}
)

ortholog_model = NS.model(
    "orthologs",
    {
...
ortholog_algorithms_model = NS.model(
    "ortholog_algorithms",
    {
...
class get_orthologs_by_to_and_from_gene(Resource):
    ...
    def get(self, from_ode_ref_id, from_ode_gene_id, to_ode_ref_id, to_ode_gene_id):
        ...
        to_agr_gn_id = (
            db.query(Gene).filter(Gene.gn_ref_id == to_gn_ref).first()
        ).gn_id
        from_agr_gn_id = (
            db.query(Gene).filter(Gene.gn_ref_id == from_gn_ref).first()
        ).gn_id

        result = (
            db.query(Ortholog)
            .filter(
                Ortholog.from_gene == from_agr_gn_id, Ortholog.to_gene == to_agr_gn_id
            )
            .all()
        )
        if not result:
            abort(
                404, message="Could not find any orthologs with that from and to gene"
            )
        return result
...
class get_orthologs_by_from_gene_and_best(Resource):
    ...
    def get(self, from_ode_ref_id, from_ode_gene_id, best):
        ...
        result = (
            db.query(Ortholog)
            .filter(
                Ortholog.from_gene == agr_gn_id, Ortholog.ort_is_best == modified_best
            )
            .all()
        )
        if not result:
            abort(
                404,
                message="Could not find any orthologs with that from gene and ort_is_best value",
            )
        return result
...
class get_orthologs_by_from_to_gene_and_best(Resource):
    ...
    @NS.marshal_with(ortholog_model)
    def get(
        self, from_ode_ref_id, from_ode_gene_id, to_ode_ref_id, to_ode_gene_id, best
    ):
        # best variable is a string, must convert it to a bool to use in query
        best = best.upper()
        if best == "FALSE" or best == "F":
        ...
        result = (
            db.query(Ortholog)
            .filter(
                Ortholog.from_gene == from_agr_gn_id,
                Ortholog.to_gene == to_agr_gn_id,
                Ortholog.ort_is_best == modified_best,
            )
            .all()
        )
        if not result:
            abort(
                404,
                message="Could not find any orthologs with that "
...
class get_orthologs_by_from_to_gene_and_revised(Resource):
    ...
    @NS.doc(
        "returns all orthologs from and to specified gene and by the ort_best_revised variable"
    )
    @NS.marshal_with(ortholog_model)
    def get(
        self,
        ...
        result = (
            ...
            .filter(
                Ortholog.from_gene == from_agr_gn_id,
                Ortholog.to_gene == to_agr_gn_id,
                Ortholog.ort_is_best_revised == inp,
            )
            .all()
        )
        if not result:
            abort(
                404,
                message="Could not find any orthologs with that from_gene,"
...
class get_orthologs_by_num_algorithms(Resource):
    ...
    def get(self, num):
        result = (
            db.query(Ortholog)
            .filter(Ortholog.ort_num_possible_match_algorithms == num)
            .all()
        )
        if not result:
            abort(
                404,
                message="Could not find any orthologs with that number "
...
class get_ortholog_by_algorithm(Resource):
    ...
    def get(self, alg_name):
        ...
        alg_id = (
            db.query(Algorithm.alg_id).filter(Algorithm.alg_name == alg_name).first()
        )
        orthologs = (
            db.query(OrthologAlgorithms)
            .filter(OrthologAlgorithms.alg_id == alg_id)
            .all()
        )
        return orthologs
...
class get_homology_by_id_and_species(Resource):
    ...
    def get(self, hom_id, sp_id):
        homologs = (
            db.query(Homology)
            .filter(Homology.hom_id == hom_id, Homology.sp_id == sp_id)
            .all()
        )
        if not homologs:
...
class get_homology_by_id_and_source(Resource):
    ...
    def get(self, hom_id, hom_source_name):
        homologs = (
            db.query(Homology)
            .filter(
                Homology.hom_id == hom_id, Homology.hom_source_name == hom_source_name
            )
            .all()
        )
        if not homologs:
            abort(
                404,
                message="There are not homologs with given hom_id and hom_source_name",
            )
        else:
            return homologs
...
class get_homology_by_gene_and_source(Resource):
    ...
    def get(self, gn_id, hom_source_name):
        homologs = (
            db.query(Homology)
            .filter(
                Homology.gn_id == gn_id, Homology.hom_source_name == hom_source_name
            )
            .all()
        )
        if not homologs:
            abort(
                404,
                message="There are not homologs with given gn_id and hom_source_name",
            )
        else:
            return homologs
...
class get_ortholog_by_to_and_from_species(Resource):
    ...
    def get(self, to_sp_name, from_sp_name):
        ...
        from_sp_id = (
            db.query(Species.sp_id).filter(Species.sp_name == from_sp_name).first()
        )
        ...
        orthos = (
            db.query(Ortholog)
            .filter(
                Ortholog.to_gene.in_(to_gn_ids), Ortholog.from_gene.in_(from_gn_ids)
            )
            .all()
        )
        if not orthos:
...
class get_orthologous_species(Resource):
    ...
    def get(self, ode_gene_id, ode_ref_id):
        ...
        orthologous_from_genes = (
            db.query(Ortholog.from_gene).filter(Ortholog.to_gene == agr_gene_id).all()
        )
        orthologous_to_genes = (
            db.query(Ortholog.to_gene).filter(Ortholog.from_gene == agr_gene_id).all()
        )
        ...
        for o in all_orthologs:
            species.append(
                convert_species_agr_to_ode(
                    db.query(Gene.sp_id).filter(Gene.gn_id == o).first()[0]
                )
            )
...
class get_ortholog_by_to_from_species_and_algorithm(Resource):
    ...
    def get(self, to_sp_name, from_sp_name, alg_name):
        ...
        orthos_algorithm = (
            db.query(OrthologAlgorithms)
            .filter(OrthologAlgorithms.alg_id == algo_id)
            .all()
        )
        ...
        orthos = (
            db.query(Ortholog)
            .filter(
                Ortholog.to_gene.in_(to_gn_ids),
                Ortholog.from_gene.in_(from_gn_ids),
                Ortholog.ort_id.in_(ort_ids),
            )
            .all()
        )
...
class agr_to_geneweaver_species(Resource):
    ...
    @NS.doc(
        "translate an AGR species id to the corresponding species id in the geneweaver database"
    )
    def get(self, sp_id):
...
class get_ode_gene_by_species(Resource):
    ...
    def get(self, ode_gene_id, sp_name):
        ...
        if not genes:
            abort(
                404, message="No genes were found matching that ode_gene_id and species"
            )
        return genes
...
class get_ort_id_if_gene_is_ortholog(Resource):
    ...
    @NS.doc(
        "returns the ortholog id filtering by the ortholog from gene, including the gdb_id"
    )
    def get(self, ode_gene_id, ode_ref_id):
        ...
        ort_id = (
            db.query(Ortholog.ort_id).filter(
                (Ortholog.from_gene == gn_id) | (Ortholog.to_gene == gn_id)
            )
        ).all()
        return ort_id
...
class get_homologous_ode_gene_ids_for_gene(Resource):
    ...
    def get(self, ode_ref_id, gdb_name):
        ...
        homologous_gn_ids = (
            db.query(Homology.gn_id).filter(Homology.hom_id.in_(hom_ids)).all()
        )
        homologous_gn_ids = [l[0] for l in set(homologous_gn_ids)]

        homologous_agr_refs = (
            db.query(Gene.gn_ref_id).filter(Gene.gn_id.in_(homologous_gn_ids)).all()
        )
        homologous_refs = [
            convert_agr_ref_to_ode(l[0]) for l in set(homologous_agr_refs)
        ]
...
class get_ode_genes_from_hom_id(Resource):
    ...
    def get(self, hom_id, target_gdb_id):
        ...
        homologous_gn_ids = (
            db.query(Homology.gn_id)
            .filter(Homology.hom_id == hom_id, Homology.sp_id == target_sp_id)
            .all()
        )
        ...
        for id in homologous_gn_ids:
            ref = convert_agr_ref_to_ode(
                (db.query(Gene.gn_ref_id).filter(Gene.gn_id == id).first())[0]
            )
            if not ref:
                break
            refs.append(ref)
...
class get_ortholog_by_from_gene_and_gdb(Resource):
    ...
    @NS.doc(
        "returns to gene ode_gene_id and ode_ref_id of any ortholog with the from gene matching"
    ...
    def get(self, from_ode_gene_id, gdb_id):
        ...
        from_gene_ids = (
            db.query(Gene.gn_id).filter(Gene.gn_ref_id.in_(from_gene_refs)).all()
        )

        # a list of to_gene ids where the from_gene value is in the from_genes list of ids
        to_gene_ids = (
            db.query(Ortholog.to_gene)
            .filter(Ortholog.from_gene.in_(from_gene_ids))
            .all()
        )

        # agr ref ids of to_genes
        to_gene_refs = (
            db.query(Gene.gn_ref_id)
            .filter(Gene.gn_id.in_(to_gene_ids), Gene.sp_id == agr_species)
            .all()
        )
...
class get_intersect_by_homology(Resource):
    ...
    def get(self):
        ...
        hom_and_gn_id1 = (
            db.query(Homology.hom_id, Homology.gn_id)
            .filter(Homology.gn_id.in_(gn_ids1))
            .all()
        )
        hom_and_gn_id2 = (
            db.query(Homology.hom_id, Homology.gn_id)
            .filter(Homology.gn_id.in_(gn_ids2))
            .all()
        )
...
class transpose_genes_by_species(Resource):
    ...
    def get(self):
        ...
        homologous_new_species_gn_ids = (
            db.query(Homology.gn_id)
            .filter(Homology.hom_id.in_(hom_ids), Homology.sp_id == sp)
            .all()
        )
        homologous_new_species_gn_ids = list(
            set(list(zip(*homologous_new_species_gn_ids))[0])
        )

        # convert gn_ids back to ode_ref_ids
        homologous_new_species_agr_refs = (
            db.query(Gene.gn_ref_id)
            .filter(Gene.gn_id.in_(homologous_new_species_gn_ids))
            .all()
        )
        homologous_new_species_agr_refs = list(
            set(list(zip(*homologous_new_species_agr_refs))[0])
        )
...
class get_orthologs_by_symbol(Resource):
    ...
    def get(self, sym, orig_species, homologous_species):
        ...
        homologous_sp_id = (
            db.query(Species.sp_id)
            .filter(Species.sp_name == homologous_species)
            .first()
        )
        ...
        for s in symbols:
            ...
            orthos = (
                db.query(Ortholog.to_gene).filter(Ortholog.from_gene == agr_id).all()
            )
            orthos.extend(
                db.query(Ortholog.from_gene).filter(Ortholog.to_gene == agr_id).all()
            )
            ...
            agr_ortho_refs = (
                db.query(Gene.gn_ref_id)
                .filter(Gene.gn_id.in_(orthos), Gene.sp_id == homologous_sp_id)
                .all()
            )
            # format the list
            ortho_refs = []
