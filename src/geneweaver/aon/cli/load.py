...
def gw():
    ...
    with Progress(transient=True) as progress:
        ...
        # Genes
        with psycopg.connect(config.GW_DB.URI.replace('postgresql+psycopg', 'postgresql')) as connection:
        ...
        # Homologs
        with psycopg.connect(config.GW_DB.URI) as gw_connection, psycopg.connect(config.DB.URI) as aon_connection:
            with gw_connection.cursor() as gw_cursor, aon_connection.cursor() as aon_cursor:
                homologs = geneweaver.homologs.get_homolog_information(aon_cursor, gw_cursor)
